import { ButtonComponent, ColorInputComponent, DropdownInputComponent, FieldSetComponent, FieldSetDropdownComponent, FieldSetToggleComponent, FontInputComponent, InfoBoxComponent, RangeInputComponent, setupMenuTabs, TextComponent, TextInputComponent } from './components.js';
import { ImageSamplerDistanceMethod } from './sdf.js';
import { TextControl } from './text.js';
export function setupUI(scene, textControl, inputControl) {
    setupMenuTabs();
...
function createInputSettings(textControl, inputControl) {
    const result = [];
    ...
    const inputSourceSettingsFS = new FieldSetDropdownComponent('Source', index => {
        inputControl.sdfSource = index;
    });
    const textInput = new TextInputComponent('Text', 'Aybg3iTpÑl1Á', 'Aybg3iTpÑl1Á', t => inputControl.textSettings.text = t);
    ...
    const sourceSizeInfo = (width, height) => {
...
function createSceneSettings(scene) {
    const result = [];
    const backgroundInput = new ColorInputComponent('Background', '#000000', '#000000', c => scene.setBackground(c));
    const settingsFS = new FieldSetComponent('Settings');
    settingsFS.append(backgroundInput);
    result.push(settingsFS);
    const viewDistanceInput = new RangeInputComponent('Distance', 1.25, 1.25, 0.25, 20.0, 0.01, v => scene.setViewDistance(v));
    const viewXInput = new RangeInputComponent('Horizontal offset', 0.0, 0.0, -1.0, 1.0, 0.01, v => scene.setViewX(-v));
    const viewYInput = new RangeInputComponent('Vertical offset', 0.0, 0.0, -1.0, 1.0, 0.01, v => scene.setViewY(-v));
...
function createStylesBasicSettings(textControl) {
    const fontWeightInput = new RangeInputComponent('Weight', TextControl.WeightDefault, TextControl.WeightDefault, 0.3, 0.6, 0.01, v => textControl.setWeight(v));
    const opacityInput = new RangeInputComponent('Opacity', TextControl.OpacityDefault, TextControl.OpacityDefault, 0.0, 1.0, 0.01, v => textControl.setOpacity(v));
    const blurInput = new RangeInputComponent('Blur', TextControl.BlurDefault * 2.0, TextControl.BlurDefault * 2.0, 0.0, 1.0, 0.01, v => textControl.setBlur(v));
    const fillColorInput = new ColorInputComponent('', TextControl.ColorDefault, TextControl.ColorDefault, c => {
        textControl.setColor(c);
        textControl.setColorB(c);
    });
    const fillGradientTopInput = new ColorInputComponent('Top', TextControl.ColorDefault, TextControl.ColorDefault, c => textControl.setColor(c));
    const fillGradientBottomInput = new ColorInputComponent('Bottom', TextControl.ColorBDefault, TextControl.ColorBDefault, c => textControl.setColorB(c));
    const fillGradientScaleInput = new RangeInputComponent('Scale', TextControl.ColorScaleDefault, TextControl.ColorScaleDefault, 0.0, 1.0, 0.01, v => textControl.setColorScale(v));
    const fillGradientOffsetInput = new RangeInputComponent('Offset', TextControl.ColorOffsetDefault, TextControl.ColorOffsetDefault, -0.5, 0.5, 0.01, v => textControl.setColorOffset(v));
...
function createStylesStrokeSettings(textControl) {
    const fillColorInput = new ColorInputComponent('', TextControl.BorderColorDefault, TextControl.BorderColorDefault, c => {
        textControl.setBorderColor(c);
        textControl.setBorderColorB(c);
    });
    const fillGradientTopInput = new ColorInputComponent('Top', TextControl.BorderColorDefault, TextControl.BorderColorDefault, c => textControl.setBorderColor(c));
    const fillGradientBottomInput = new ColorInputComponent('Bottom', TextControl.BorderColorBDefault, TextControl.BorderColorBDefault, c => textControl.setBorderColorB(c));
    const fillGradientScaleInput = new RangeInputComponent('Scale', TextControl.BorderColorScaleDefault, TextControl.BorderColorScaleDefault, 0.0, 1.0, 0.01, v => textControl.setBorderColorScale(v));
    const fillGradientOffsetInput = new RangeInputComponent('Offset', TextControl.BorderColorOffsetDefault, TextControl.BorderColorOffsetDefault, -0.5, 0.5, 0.01, v => textControl.setBorderColorOffset(v));
    const fillFS = new FieldSetDropdownComponent('Fill');
    fillFS.addOption('Color', fillColorInput);
    fillFS.addOption('Gradient', fillGradientTopInput, fillGradientBottomInput, fillGradientScaleInput, fillGradientOffsetInput);
    const thicknessInput = new RangeInputComponent('Thickness', TextControl.BorderThicknessDefault, TextControl.BorderThicknessDefault, -0.5, 0.5, 0.01, v => textControl.setBorderThickness(v));
    const container = new FieldSetToggleComponent('Stroke');
    container.append(fillFS);
    container.append(thicknessInput);
    return container;
}
function createStylesDropShadowSettings(textControl) {
    const colorInput = new ColorInputComponent('Color', TextControl.ShadowColorDefault, TextControl.ShadowColorDefault, c => textControl.setShadowColor(c));
    const opacityInput = new RangeInputComponent('Opacity', TextControl.ShadowOpacityDefault, 0.5, 0.0, 1.0, 0.01, v => textControl.setShadowOpacity(v));
    const blurInput = new RangeInputComponent('Blur', TextControl.ShadowBlurDefault * 2.0, TextControl.ShadowBlurDefault * 2.0, 0.0, 1.0, 0.01, v => textControl.setShadowBlur(v));
    const horizontalOffsetInput = new RangeInputComponent('H. offset', TextControl.ShadowOffsetXDefault, TextControl.ShadowOffsetXDefault, -1.0, 1.0, 0.02, v => textControl.setShadowOffsetX(v));
    const verticalOffsetInput = new RangeInputComponent('V. offset', TextControl.ShadowOffsetYDefault, TextControl.ShadowOffsetYDefault, -1.0, 1.0, 0.02, v => textControl.setShadowOffsetY(v));
...
function createStylesInnerShadowSettings(textControl) {
    const colorInput = new ColorInputComponent('Color', TextControl.InnerShadowColorDefault, TextControl.InnerShadowColorDefault, c => textControl.setInnerShadowColor(c));
    const opacityInput = new RangeInputComponent('Opacity', TextControl.InnerShadowOpacityDefault, 0.5, 0.0, 1.0, 0.01, v => textControl.setInnerShadowOpacity(v));
    const blurInput = new RangeInputComponent('Blur', TextControl.InnerShadowBlurDefault * 2.0, TextControl.InnerShadowBlurDefault * 2.0, 0.0, 1.0, 0.01, v => textControl.setInnerShadowBlur(v));
    const horizontalOffsetInput = new RangeInputComponent('H. offset', TextControl.InnerShadowOffsetXDefault, TextControl.InnerShadowOffsetXDefault, -1.0, 1.0, 0.02, v => textControl.setInnerShadowOffsetX(v));
    const verticalOffsetInput = new RangeInputComponent('V. offset', TextControl.InnerShadowOffsetYDefault, TextControl.InnerShadowOffsetYDefault, -1.0, 1.0, 0.02, v => textControl.setInnerShadowOffsetY(v));
...
function createStylesBevelSettings(textControl) {
    const warning = new InfoBoxComponent('Experimental feature');
    const lightDirInput = new RangeInputComponent('Light direction', 0, 0, 0, 359, 1, v => textControl.setBevelLightDir(v));
    const opacityInput = new RangeInputComponent('Opacity', TextControl.BevelOpacityDefault, 0.5, 0.0, 1.0, 0.01, v => textControl.setBevelOpacity(v));
    const blurInput = new RangeInputComponent('Blur', TextControl.BevelBlurDefault * 2.0, TextControl.BevelBlurDefault * 2.0, 0.0, 1.0, 0.01, v => textControl.setBevelBlur(v));
    const thicknessInput = new RangeInputComponent('Thickness', TextControl.BevelThicknessDefault, 0.1, 0.0, 0.45, 0.01, v => textControl.setBevelThickness(v));
